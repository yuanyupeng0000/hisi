...
void SAMPLE_SVP_HandleSig(int s32Signo)
{
    ...
    if (SIGINT == s32Signo || SIGTERM == s32Signo)
    {
        switch (*s_ppChCmdArgv[1])
        {
            ...
            case 'b':
                {
                   SAMPLE_SVP_NNIE_Tiny_Yolov3_HandleSig();
                }
            break;
            case 'c':
                {
                   SAMPLE_SVP_NNIE_Tiny_Yolov3_ViVpssVencVo_HandleSig();
                }
            break;
            case 'd':
                {
            ...
            case 'e':
                {
                   SAMPLE_SVP_NNIE_Yolov3_ViVpssVencVo_HandleSig();
                }
            break;
           default :
            {
...
void SAMPLE_SVP_Usage(char* pchPrgName)
{
    ...
    printf("\t b) Yolov3Tiny(Read File).\n");
    printf("\t c) Yolov3Tiny(VI->VPSS->NNIE->VGS->VO).\n");
    printf("\t d) Yolov3(Read File).\n");
    printf("\t e) Yolov3(VI->VPSS->NNIE->VGS->VO).\n");
}
...
#endif
{
    ...
    switch (*argv[1])
    {
        ...
        case '5':
            {
                //SAMPLE_SVP_NNIE_Ssd();
                SAMPLE_SVP_NNIE_SSD_LOOP_SPLIT();
                //SAMPLE_SVP_NNIE_SSD_LOOP();
            }
            break;
        case '6':
            {
        ...
        case 'b':
            {
                //SAMPLE_SVP_NNIE_Tiny_Yolov3();
                SAMPLE_SVP_NNIE_YOLOV3_TINY_READ_FILE_LOOP();
            }
        break;
        case 'c':
            {
                SAMPLE_SVP_NNIE_YOLOV3_TINY();
            }
        break;
        case 'd':
            {
                SAMPLE_SVP_NNIE_YOLOV3_READ_FILE_LOOP();
            }
        break;
        case 'e':
            {
                //SAMPLE_SVP_NNIE_YOLOV3();
                SAMPLE_SVP_NNIE_YOLOV3_API();
            }
        break;
        default :
            {
