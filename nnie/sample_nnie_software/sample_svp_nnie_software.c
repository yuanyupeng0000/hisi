...
/*****************************************************************************
*   Prototype    : SVP_NNIE_Yolov3_Tiny_GetResult
*   Description  : Yolov3 Tiny GetResult function
* Input :     HI_S32    *ps32InputData    [IN]  pointer to the input data memory
*              HI_U32    u32GridNumWidth   [IN]  Grid num in width direction
*              HI_U32    u32GridNumHeight  [IN]  Grid num in height direction
*              HI_U32    u32EachGridBbox   [IN]  Bbox num of each gird
...
*
*****************************************************************************/
static HI_S32 SVP_NNIE_Yolov3_Tiny_GetResult(HI_S32 *ps32InputData,HI_U32 u32GridNumWidth,
    HI_U32 u32GridNumHeight,HI_U32 u32EachGridBbox,HI_U32 u32ClassNum,HI_U32 u32SrcWidth,
    HI_U32 u32SrcHeight,HI_U32 u32MaxRoiNum,HI_U32 u32NmsThresh,HI_U32 u32ConfThresh,
    HI_FLOAT af32Bias[],HI_U32* pu32TmpBuf,HI_S32 *ps32DstScores, HI_S32 *ps32DstRoi,
    HI_S32 *ps32ClassRoiNum)
{
    ...
    for(n = 0;n < u32GridNum ;n++)
    {
        ...
        for(k = 0;k <u32EachGridBbox;k++)
        {
            ...
            x = (HI_FLOAT)((w + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 0])) / u32GridNumWidth); // x
            y = (HI_FLOAT)((h + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 1])) / u32GridNumHeight); // y
            f32Width = (HI_FLOAT)((exp(pf32BoxTmp[u32Index + 2]) * af32Bias[2 * k]) / 416); // w
            f32Height =(HI_FLOAT)((exp(pf32BoxTmp[u32Index + 3]) * af32Bias[2 * k + 1]) / 416); // h

            f32ObjScore = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 4]);
            ///SVP_NNIE_SoftMax(&pf32BoxTmp[u32Index + 5],u32ClassNum);
            for(HI_U32 i=0; i<u32ClassNum; i++){
                pf32BoxTmp[u32Index + 5 + i] = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 5 + i]);
            }
...
*
*****************************************************************************/
static HI_S32 SVP_NNIE_Yolov3_Tiny_GetResult_(SVP_DST_BLOB_S *pstDstBlob,HI_U32 u32GridNumWidth[],
    HI_U32 u32GridNumHeight[],HI_U32 u32EachGridBbox,HI_U32 u32ClassNum,HI_U32 u32SrcWidth,
    HI_U32 u32SrcHeight,HI_U32 u32MaxRoiNum,HI_U32 u32NmsThresh,HI_U32 u32ConfThresh,
    HI_FLOAT af32Bias[][6],HI_U32* pu32TmpBuf,HI_S32 *ps32DstScores, HI_S32 *ps32DstRoi,
    HI_S32 *ps32ClassRoiNum)
{
    HI_U8 u8LayerId = 0;
    HI_U32 u32GridNum = 0;
    ...
    HI_U32 u32CStep = u32GridNum;
    HI_U32 u32HStep = u32GridNumWidth[u8LayerId];
    HI_U32 u32BoxsNum = 0;
    HI_FLOAT *pf32BoxTmp[2] = {NULL};
    HI_FLOAT *f32InputData = NULL;
    HI_FLOAT f32ObjScore = 0.0;
    HI_FLOAT f32MaxScore = 0.0;
    HI_S32 s32Score = 0;
    HI_U32 u32MaxValueIndex = 0;
    HI_U32 h = 0,w = 0,n = 0, m = 0, q=0;
    HI_U32 c = 0,k = 0,i=0;
    HI_U32 u32Index= 0;
    HI_FLOAT x,y,f32Width,f32Height;
    HI_U32 u32AssistBuffSize = u32TotalBboxNum * sizeof(SAMPLE_SVP_NNIE_STACK_S);
    ...
    HI_S32 *ps32InputData = NULL;
    HI_U32 u32ThisLayerBboxNum = 0;
    HI_U32 u32BlobElementNum = 0;
    HI_U32 u32BlobWidthElementNum = 0;
    HI_U32 u32Yolov3TinyOutputLayerNum = 2;


    for(HI_U8 t=0; t<u32Yolov3TinyOutputLayerNum; t++)
    {
        u8LayerId = t;
        //printf("\n[ INFO ]: Start parsing \n");
        printf("\n%d;%d;%d;%d\n", pstDstBlob[t].unShape.stWhc.u32Width, pstDstBlob[t].unShape.stWhc.u32Height,
               pstDstBlob[t].unShape.stWhc.u32Chn, pstDstBlob[t].u32Stride);
        u32BlobElementNum = pstDstBlob[t].unShape.stWhc.u32Chn * pstDstBlob[t].unShape.stWhc.u32Height
                * pstDstBlob[t].u32Stride/sizeof(HI_S32);
        u32BlobWidthElementNum = pstDstBlob[t].u32Stride/sizeof(HI_S32);
        w = pstDstBlob[t].unShape.stWhc.u32Width;
        ps32InputData = SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstDstBlob[t].u64VirAddr);
        u32GridNum = u32GridNumWidth[u8LayerId]*u32GridNumHeight[u8LayerId];
        //u32TotalBboxNum = u32GridNum*u32EachGridBbox;
        u32TotalBboxNum = u32GridNumWidth[0]*u32GridNumHeight[0]*u32EachGridBbox +
                u32GridNumWidth[1]*u32GridNumHeight[1]*u32EachGridBbox;
        u32ThisLayerBboxNum = u32GridNum*u32EachGridBbox;
        u32CStep = u32GridNum;
        u32HStep = u32GridNumWidth[u8LayerId];
        u32AssistBuffSize = u32TotalBboxNum * sizeof(SAMPLE_SVP_NNIE_STACK_S);
        u32BoxBuffSize = u32TotalBboxNum * sizeof(SAMPLE_SVP_NNIE_YOLOV2_BBOX_S);
        /*store float type data*/
        f32InputData = (HI_FLOAT*)pu32TmpBuf;
        /*assist buffer for sort*/
        pstAssistStack = (SAMPLE_SVP_NNIE_STACK_S*)(f32InputData+u32TotalBboxNum*u32ParaNum);
        ...
        pstBox = (SAMPLE_SVP_NNIE_YOLOV2_BBOX_S*)((HI_U8*)pstAssistStack+u32AssistBuffSize);
        /*box tmp buffer*/
        pf32BoxTmp[t] = (HI_FLOAT*)((HI_U8*)pstBox + u32BoxBuffSize) + m;
        //printf("||\n");
        q = 0;

        ///remove the padded element according to stride size
        for(i = 0;i < u32BlobElementNum /*u32ThisLayerBboxNum*u32ParaNum + 36*13*3*/;i++)
        {
            //f32InputData[i] = (HI_FLOAT)(ps32InputData[i])/SAMPLE_SVP_NNIE_QUANT_BASE;
            if(i%u32BlobWidthElementNum >= w/*i%16 == 13 || i%16 == 14 || i%16 == 15*/){
                continue;
            }
            f32InputData[q] = (HI_FLOAT)(ps32InputData[i])/SAMPLE_SVP_NNIE_QUANT_BASE;
            /*if(q%w == 0){
                printf("\n||\n");
            }
            if(q%(w*w)==0){
        ...
        //case 13:
            ...
            //break;
        //case 26:
            //for(i = 0;i < u32BlobElementNum /*u32ThisLayerBboxNum*u32ParaNum + 36*26*2*/;i++)
            //{
                //f32InputData[i] = (HI_FLOAT)(ps32InputData[i])/SAMPLE_SVP_NNIE_QUANT_BASE;
                //if(i%28 == 26 || i%28 == 27){
                //    continue;
            ...
            //break;
        //case 52:
            //for(i = 0;i < u32BlobElementNum /*u32ThisLayerBboxNum*u32ParaNum*/;i++)
            //{
                //f32InputData[i] = (HI_FLOAT)(ps32InputData[i])/SAMPLE_SVP_NNIE_QUANT_BASE;
                /*if(i%52 == 0)
                {
                ...
                printf("%f ", f32InputData[i]);*/
            //}
            //break;

        //default:
        //    printf("\n[ ERROR ]: Output dimension unsupport !\n");
        //    return HI_FAILURE;
        //    break;
        //}
        ...
        //permute
        m = 0;
        for(h = 0; h< u32GridNumHeight[u8LayerId];h++)
        {
            for(w = 0;w < u32GridNumWidth[u8LayerId];w++)
            {
                for(c = 0;c < u32EachGridBbox*u32ParaNum;c++)
                {
                    pf32BoxTmp[t][m++] = f32InputData[c*u32CStep + h*u32HStep + w];
                }
            }
        }
        ...
        {
            //Grid
            w = n % u32GridNumWidth[u8LayerId];
            h = n / u32GridNumWidth[u8LayerId];
            for(k = 0;k <u32EachGridBbox;k++)
            {
                u32Index = (n * u32EachGridBbox + k) * u32ParaNum;
                x = (HI_FLOAT)((w + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 0])) / u32GridNumWidth[u8LayerId]); // x
                y = (HI_FLOAT)((h + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 1])) / u32GridNumHeight[u8LayerId]); // y
                f32Width = (HI_FLOAT)((exp(pf32BoxTmp[t][u32Index + 2]) * af32Bias[u8LayerId][2 * k]) / 416); // w
                f32Height =(HI_FLOAT)((exp(pf32BoxTmp[t][u32Index + 3]) * af32Bias[u8LayerId][2 * k + 1]) / 416); // h

                f32ObjScore = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 4]);
                ///SVP_NNIE_SoftMax(&pf32BoxTmp[u32Index + 5],u32ClassNum);
                for(HI_U32 i=0; i<u32ClassNum; i++){
                    pf32BoxTmp[t][u32Index + 5 + i] = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 5 + i]);
                }

                f32MaxScore = SVP_NNIE_Yolov2_GetMaxVal(&pf32BoxTmp[t][u32Index + 5],u32ClassNum,&u32MaxValueIndex);
...
*
*****************************************************************************/
static HI_S32 SVP_NNIE_Yolov3_GetResult(SVP_DST_BLOB_S *pstDstBlob,HI_U32 u32GridNumWidth[],
    HI_U32 u32GridNumHeight[],HI_U32 u32EachGridBbox,HI_U32 u32ClassNum,HI_U32 u32SrcWidth,
    HI_U32 u32SrcHeight,HI_U32 u32MaxRoiNum,HI_U32 u32NmsThresh,HI_U32 u32ConfThresh,
    HI_FLOAT af32Bias[][6],HI_U32* pu32TmpBuf,HI_S32 *ps32DstScores, HI_S32 *ps32DstRoi,
    HI_S32 *ps32ClassRoiNum)
{
    ...
    HI_U32 u32Index= 0;
    HI_FLOAT x,y,f32Width,f32Height;
    HI_U32 u32AssistBuffSize = 0;
    HI_U32 u32BoxBuffSize = 0;
    HI_U32 u32BoxResultNum = 0;
    SAMPLE_SVP_NNIE_STACK_S *pstAssistStack = NULL;
    SAMPLE_SVP_NNIE_YOLOV2_BBOX_S *pstBox = NULL;

    HI_U32 u32ParaNum = (SAMPLE_SVP_NNIE_COORDI_NUM+1+u32ClassNum);
    HI_U32 u32TotalGridNum = 0;
    HI_U32 u32HStep = 0;
    HI_U32 u32CStep = 0;
    HI_U32 u32TotalBboxNum = 0;
    HI_U32 u32ThisLayerBboxNum = 0;
    HI_U32 u32ThisLayerGridNum = 0;
    HI_S32 *ps32InputData = NULL;
    HI_U8 u8LayerId = 0;
    HI_U32 u32BlobElementNum = 0;
    HI_U32 u32BlobWidthElementNum = 0;
    ...
    u32TotalGridNum =
            u32GridNumWidth[0]*u32GridNumHeight[0] +
            u32GridNumWidth[1]*u32GridNumHeight[1] +
            u32GridNumWidth[2]*u32GridNumHeight[2];
    u32TotalBboxNum = u32TotalGridNum*u32EachGridBbox;
    u32AssistBuffSize = u32TotalBboxNum * sizeof(SAMPLE_SVP_NNIE_STACK_S);
    u32BoxBuffSize = u32TotalBboxNum * sizeof(SAMPLE_SVP_NNIE_YOLOV2_BBOX_S);
    /*store float type data*/
    f32InputData = (HI_FLOAT*)pu32TmpBuf;
    ...
    for (HI_U32 t=0; t<3; t++)
    {
        u8LayerId = t;
        printf("\n%d;%d;%d;%d\n", pstDstBlob[t].unShape.stWhc.u32Width, pstDstBlob[t].unShape.stWhc.u32Height,
               pstDstBlob[t].unShape.stWhc.u32Chn, pstDstBlob[t].u32Stride);
        ps32InputData = SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstDstBlob[t].u64VirAddr);
        u32ThisLayerGridNum = u32GridNumWidth[u8LayerId]*u32GridNumHeight[u8LayerId];
        u32ThisLayerBboxNum = u32ThisLayerGridNum*u32EachGridBbox;
        u32CStep = u32ThisLayerGridNum;
        u32HStep = u32GridNumWidth[u8LayerId];
        u32BlobElementNum = pstDstBlob[t].unShape.stWhc.u32Chn * pstDstBlob[t].unShape.stWhc.u32Height
                * pstDstBlob[t].u32Stride/sizeof(HI_S32);
        u32BlobWidthElementNum = pstDstBlob[t].u32Stride/sizeof(HI_S32);
       ...
       //permute
       m = 0;
       for(h = 0; h < u32GridNumHeight[t];h++)
       {
           for(w = 0;w < u32GridNumWidth[t];w++)
           {
               for(c = 0;c < u32EachGridBbox*u32ParaNum;c++)
               {
                   pf32BoxTmp[t][m++] = f32InputData[c*u32CStep + h*u32HStep + w];
       ...
       for(n = 0;n < u32ThisLayerGridNum ;n++)
       {
           //Grid
           w = n % u32GridNumWidth[t];
           h = n / u32GridNumWidth[t];
           for(k = 0; k < u32EachGridBbox; k++)
           {
               u32Index = (n * u32EachGridBbox + k) * u32ParaNum;
               x = (HI_FLOAT)((w + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 0])) / u32GridNumWidth[t]); // x
               y = (HI_FLOAT)((h + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 1])) / u32GridNumHeight[t]); // y
               f32Width = (HI_FLOAT)((exp(pf32BoxTmp[t][u32Index + 2]) * af32Bias[t][2 * k]) / 416); // w
               f32Height =(HI_FLOAT)((exp(pf32BoxTmp[t][u32Index + 3]) * af32Bias[t][2 * k + 1]) / 416); // h
               //printf("\naf32Bias[%d][2*%d]=%f\n", t, k, af32Bias[t][2*k]);

               f32ObjScore = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 4]);
               ///SVP_NNIE_SoftMax(&pf32BoxTmp[u32Index + 5],u32ClassNum);
               for(HI_U32 j=0; j<u32ClassNum; j++){
                   pf32BoxTmp[t][u32Index + 5 + j] = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[t][u32Index + 5 + j]);
               }
...
/*****************************************************************************
* Prototype :   SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResultTmpBuf
* Description : this function is used to Get Yolov2 GetResult tmp buffer size
* Input :     SAMPLE_SVP_NNIE_PARAM_S*               pstNnieParam     [IN]  the pointer to YOLOV2 NNIE parameter
*              SAMPLE_SVP_NNIE_YOLOV2_SOFTWARE_PARAM_S*   pstSoftwareParam [IN]  the pointer to YOLOV2 software parameter
*
...
*
*****************************************************************************/
HI_U32 SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResultTmpBuf(SAMPLE_SVP_NNIE_PARAM_S*pstNnieParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    HI_U32 u32TotalGridNum = pstSoftwareParam->u32GridNumHeight*pstSoftwareParam->u32GridNumWidth;
    HI_U32 u32ParaLength = pstSoftwareParam->u32BboxNumEachGrid*(SAMPLE_SVP_NNIE_COORDI_NUM+1+pstSoftwareParam->u32ClassNum);
    HI_U32 u32TotalBboxNum = u32TotalGridNum*pstSoftwareParam->u32BboxNumEachGrid;
...
*
*****************************************************************************/
HI_U32 SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResultTmpBuf_(SAMPLE_SVP_NNIE_PARAM_S*pstNnieParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    HI_U32 u32TotalGridNum = pstSoftwareParam->u32GridNumHeightArray[0]*pstSoftwareParam->u32GridNumWidthArray[0] +
...
/*****************************************************************************
* Prototype :   SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResult
* Description : this function is used to Get Yolov2 result
* Input :     SAMPLE_SVP_NNIE_PARAM_S*               pstNnieParam     [IN]  the pointer to YOLOV2 NNIE parameter
*              SAMPLE_SVP_NNIE_YOLOV2_SOFTWARE_PARAM_S*   pstSoftwareParam [IN]  the pointer to YOLOV2 software parameter
*
...
*
*****************************************************************************/
HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResult(SAMPLE_SVP_NNIE_PARAM_S*pstNnieParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    return SVP_NNIE_Yolov3_Tiny_GetResult_(
        //SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstNnieParam->astSegData[0].astDst[1].u64VirAddr),
        pstNnieParam->astSegData[0].astDst,
        pstSoftwareParam->u32GridNumWidthArray,
        pstSoftwareParam->u32GridNumHeightArray,
        pstSoftwareParam->u32BboxNumEachGrid,pstSoftwareParam->u32ClassNum,
        pstSoftwareParam->u32OriImWidth,
        pstSoftwareParam->u32OriImHeight,
        pstSoftwareParam->u32MaxRoiNum,pstSoftwareParam->u32NmsThresh,
    ...
    SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    return SVP_NNIE_Yolov3_GetResult(
        //SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstNnieParam->astSegData[0].astDst[2].u64VirAddr),
        pstNnieParam->astSegData[0].astDst,
        pstSoftwareParam->u32GridNumWidth,
        pstSoftwareParam->u32GridNumHeight,
        pstSoftwareParam->u32BboxNumEachGrid,pstSoftwareParam->u32ClassNum,
