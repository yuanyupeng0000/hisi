...
static pthread_t s_hWriteFileThread = 0; ///yyp test
...
/*tiny yolov3 para*/
static SAMPLE_SVP_NNIE_MODEL_S s_stTinyYolov3Model = {0};
static SAMPLE_SVP_NNIE_PARAM_S s_stTinyYolov3NnieParam = {0};
static SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S s_stTinyYolov3SoftwareParam = {0};
static SAMPLE_IVE_SWITCH_S s_stTinyYolov3Switch = {HI_FALSE,HI_FALSE};
/*yolov3 para*/
...
/*Tiny Yolov3*/
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_FORWARD_SRC_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_FORWARD_PRE_DST_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_FORWARD_AFTER_DST_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_FORWARD_OP_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_GR_SRC_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_GR_PRE_DST_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_GR_AFTER_DST_FLUSH_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_GR_OP_TIME()
#define SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_PRINT()
    ...
    SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh)
{
    ...
    ///yyp test save result file
    FILE *fp;
    fp=fopen("./det_result.txt","w+");
    HI_U32 k = 0;
    HI_S8 result[1024*1024]; //1M
    HI_S8 *p = NULL;
    p = result;
...
void SAMPLE_SVP_NNIE_FasterRcnn_DoubleRoiPooling(void)
{
    ...
    HI_CHAR *pcSrcFile = "./many_car_224_224.bgr";
...
* function : Yolo Proc
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_Proc(SAMPLE_SVP_NNIE_PARAM_S *pstParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S *pstSwParam)
{
    HI_FLOAT f32PrintResultThresh = 0.45f;
    ...
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN()
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResult(&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,TINY_YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Tiny_Yolov3_GetResult failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_END()
    ...
    /*print result, this sample has 6 classes:
     ...
     SAMPLE_SVP_TRACE_INFO("Tiny Yolov3 result:\n");
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult(&s_stTinyYolov3SoftwareParam.stDstScore,
        &s_stTinyYolov3SoftwareParam.stDstRoi, &s_stTinyYolov3SoftwareParam.stClassRoiNum,f32PrintResultThresh);

    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP()

    SAMPLE_SVP_NNIE_PERF_STAT_TINY_YOLOV3_PRINT()
TINY_YOLOV3_FAIL_0:
    return s32Ret;
}
...
{
    HI_FLOAT f32PrintResultThresh = 0.1f;
    HI_S32 s32Ret = HI_FAILURE;
    HI_U32 i = 0;
    SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S stInputDataIdx = {0};
    SAMPLE_SVP_NNIE_PROCESS_SEG_INDEX_S stProcSegIdx = {0};
...
* function : Yolo Proc
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_Proc_ViToVo(SAMPLE_SVP_NNIE_PARAM_S *pstParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S *pstSwParam, VIDEO_FRAME_INFO_S* pstExtFrmInfo,
    HI_U32 u32BaseWidth, HI_U32 u32BaseHeight)
{
    ...
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_Tiny_Proc(pstParam,pstSwParam);
    SAMPLE_SVP_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,SAMPLE_SVP_ERR_LEVEL_ERROR,
...
/******************************************************************************
* function : save yuv420sp to file
******************************************************************************/
static HI_U8 Utils_Save_YUV420_File(HI_U8 *pu8Frame, HI_U32 u32Size)
{
    FILE *fp = fopen("saved.yuv","w");
    fwrite(pu8Frame,sizeof(char),u32Size,fp);
    fclose(fp);
    //free(pu8Frame);
    //system("pause");
    return 0;
...
/******************************************************************************
* function : YUV420ToBGR24
******************************************************************************/
static HI_BOOL YUV420ToBGR24(HI_U8* pY, HI_U8* pU, HI_U8* pV, HI_U8* pRGB24, HI_S32 width, HI_S32 height)
{
    int yIdx, uIdx, vIdx;
    int offset = 0;
...
/******************************************************************************
* function : Yolo vi to vo thread entry
******************************************************************************/
static HI_VOID* SAMPLE_SVP_NNIE_Yolov3_Tiny_ViToVo(HI_VOID* pArgs)
{
    HI_S32 s32Ret;
    SAMPLE_SVP_NNIE_PARAM_S *pstParam;
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S *pstSwParam;
    ...
    //pstParam = &s_stRfcnNnieParam;
    //pstSwParam = &s_stRfcnSoftwareParam;
    pstParam = &s_stTinyYolov3NnieParam;
    pstSwParam = &s_stTinyYolov3SoftwareParam;
    ...
    while (HI_FALSE == s_bNnieStopSignal)
    {
        ...
        s32Ret = HI_MPI_VPSS_GetChnFrame(s32VpssGrp, as32VpssChn[1], &stExtFrmInfo, s32MilliSec);
        ///get the frame data yyp added
        printf("\nHeight = %d\n", stExtFrmInfo.stVFrame.u32Height);
        printf("\nWidth = %d\n", stExtFrmInfo.stVFrame.u32Width);
        printf("\nu32Stride = %d\n", stExtFrmInfo.stVFrame.u32Stride[0]);
        HI_U32 u32Size = stExtFrmInfo.stVFrame.u32Height * stExtFrmInfo.stVFrame.u32Stride[0]*3/2;
        //HI_U8 *pu8Frame = (HI_U8*)HI_MPI_SYS_Mmap(stExtFrmInfo.stVFrame.u64PhyAddr[0], u32Size);
        HI_U8 *pu8Frame = (HI_U8*)HI_MPI_SYS_MmapCache(stExtFrmInfo.stVFrame.u64PhyAddr[0], u32Size);
        Utils_Save_YUV420_File(pu8Frame, u32Size);
        HI_MPI_SYS_Munmap(pu8Frame, u32Size);
        /*for(HI_U64 i=0; i<416; i++){
            if(i%stExtFrmInfo.stVFrame.u32Width == 0){
               printf("\n");
            }
            printf("%d ", *pu8Frame);
            pu8Frame++;
        ...
        ///s32Ret = SAMPLE_SVP_NNIE_Yolo_Proc_ViToVo(pstParam,pstSwParam, &stExtFrmInfo,
        ///stBaseFrmInfo.stVFrame.u32Width,stBaseFrmInfo.stVFrame.u32Height);
        s32Ret = SAMPLE_SVP_NNIE_Yolov3_Tiny_Proc_ViToVo(pstParam,pstSwParam, &stExtFrmInfo,
        ...
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
            "HI_MPI_VO_SendFrame failed, Error(%#x)!\n", s32Ret);
        //u32Size = stBaseFrmInfo.stVFrame.u32Height * stBaseFrmInfo.stVFrame.u32Stride[0]*2;
        //pu8Frame = (HI_U8*)HI_MPI_SYS_Mmap(stBaseFrmInfo.stVFrame.u64PhyAddr[0], u32Size);
        //Utils_Save_YUV420_File(pu8Frame, u32Size);
...
static HI_VOID* SAMPLE_SVP_NNIE_Yolov3_ViToVo(HI_VOID* pArgs)
{
    ...
    while (HI_FALSE == s_bNnieStopSignal)
    {
        ...
        ///s32Ret = SAMPLE_SVP_NNIE_Yolo_Proc_ViToVo(pstParam,pstSwParam, &stExtFrmInfo,
        ///stBaseFrmInfo.stVFrame.u32Width,stBaseFrmInfo.stVFrame.u32Height);
        s32Ret = SAMPLE_SVP_NNIE_Yolov3_Proc_ViToVo(&s_stYolov3NnieParam,&s_stYolov3SoftwareParam, &stExtFrmInfo,
        stExtFrmInfo.stVFrame.u32Width,stExtFrmInfo.stVFrame.u32Height);
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, BASE_RELEASE,
            "Error(%#x),SAMPLE_SVP_NNIE_Rfcn_Proc failed!\n", s32Ret);
    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    ...
    for(HI_U8 i = 0; i < pstSoftWareParam->u32ClassNum; i++)
    {
        //pstSoftWareParam->au32ConfThresh[i] = 1;
        pstSoftWareParam->af32ScoreThr[i] = 0.1f;
    }
...
/******************************************************************************
* function : Tiny Yolov3 Deinit
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_Deinit(SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam,SAMPLE_SVP_NNIE_MODEL_S *pstNnieModel)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*hardware deinit*/
    if(pstNnieParam!=NULL)
...
/******************************************************************************
* function : Yolov3 Tiny init
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_ParamInit(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*init hardware para*/
...
{
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_tiny_low_bandwidth.wk";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_tiny_inst.wk";
    SAMPLE_SVP_NNIE_CFG_S   stNnieCfg = {0};
    SIZE_S stSize;
    PIC_SIZE_E enSize = PIC_416x416;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_CHAR acThreadName[16] = {0};

    memset(&s_stTinyYolov3Model,0,sizeof(s_stTinyYolov3Model));
    memset(&s_stTinyYolov3NnieParam,0,sizeof(s_stTinyYolov3NnieParam));
    memset(&s_stTinyYolov3SoftwareParam,0,sizeof(s_stTinyYolov3SoftwareParam));

    /******************************************
     step 1: start vi vpss vo
     ******************************************/
    s_stTinyYolov3Switch.bVenc = HI_FALSE;
    s_stTinyYolov3Switch.bVo   = HI_TRUE;
    s32Ret = SAMPLE_COMM_IVE_StartViVpssVencVo(&s_stViConfig,&s_stTinyYolov3Switch,&enSize);
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_TINY_YOLOV3_0,
    ...
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stTinyYolov3Model);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_TINY_YOLOV3_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");

    /*apcRpnDataLayerName is used to set RPN data layer name
      and search RPN input data,if user has changed network struct, please
      make sure the data layer names are correct*/
    s_stTinyYolov3NnieParam.pstModel = &s_stTinyYolov3Model.stModel;
    //s_stTinyYolov3SoftwareParam.apcRpnDataLayerName[0] = "rpn_cls_score";
    //s_stRfcnSoftwareParam.apcRpnDataLayerName[1] = "rpn_bbox_pred";
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_Tiny_ParamInit(&stNnieCfg,&s_stTinyYolov3NnieParam,
           &s_stTinyYolov3SoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_TINY_YOLOV3_1,SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    pthread_create(&s_hNnieThread, 0, SAMPLE_SVP_NNIE_Yolov3_Tiny_ViToVo, NULL);
    ...
    return;
END_TINY_YOLOV3_1:
    SAMPLE_SVP_NNIE_Rfcn_Deinit(&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam,&s_stTinyYolov3Model);
END_TINY_YOLOV3_0:
    SAMPLE_COMM_IVE_StopViVpssVencVo(&s_stViConfig,&s_stTinyYolov3Switch);
    return ;
    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    ...
    pstSoftWareParam->u32OriImWidth = pstNnieParam->astSegData[0].astSrc[0].unShape.stWhc.u32Width;
    pstSoftWareParam->u32BboxNumEachGrid = 3; //
    pstSoftWareParam->u32ClassNum = 7;//80; //
    pstSoftWareParam->u32GridNumHeight[0] = 13;
    pstSoftWareParam->u32GridNumHeight[1] = 26;
    pstSoftWareParam->u32GridNumHeight[2] = 52;
    pstSoftWareParam->u32GridNumWidth[0] = 13;
    pstSoftWareParam->u32GridNumWidth[1] = 26;
    pstSoftWareParam->u32GridNumWidth[2] = 52;
    ...
    u32ClassNum = pstSoftWareParam->u32ClassNum;
    u32BboxNum = pstSoftWareParam->u32BboxNumEachGrid * (
                pstSoftWareParam->u32GridNumHeight[0] * pstSoftWareParam->u32GridNumWidth[0] +
                pstSoftWareParam->u32GridNumHeight[1] * pstSoftWareParam->u32GridNumWidth[1] +
                pstSoftWareParam->u32GridNumHeight[2] * pstSoftWareParam->u32GridNumWidth[2]);
...
HI_BOOL query_value_of_key(HI_CHAR* cSrc, HI_CHAR* cKey, HI_CHAR* cValue){
    HI_BOOL bRet = HI_FALSE;
    HI_CHAR *cPosKey;
    HI_CHAR *cPosVal;
    cPosKey = strstr(cSrc, cKey);
    if(cPosKey != HI_NULL){
        bRet = HI_TRUE;
        cPosVal = cSrc + (strlen(cSrc) - (strlen(cPosKey) - strlen(cKey)));
        //printf("%s\n", cPosKey);
        strcpy(cValue, cPosVal);
...
HI_S32 hisi_3519av100_nnie_read_config(HI_CHAR* pcConfig, SAMPLE_SVP_NNIE_CONFIG_S* stNnie_config){
    HI_S32 s32Ret = HI_FAILURE;
    FILE * fp=fopen(pcConfig,"r");
    if(fp == HI_NULL){
        printf("[ERRO] %s not exist.", pcConfig);
        return HI_FAILURE;
    }
    HI_CHAR cbuf[1024];
    HI_CHAR ctmp[256];
    ...
    while(HI_TRUE){
        ...
        if(fgets(cbuf, sizeof(cbuf), fp) == HI_NULL){
            break;
        }
        if(HI_TRUE == query_value_of_key(cbuf, "model_wk=", stNnie_config->pcModelName)){
            printf("[INFO] model_wk: %s\n", stNnie_config->pcModelName);

            if(fopen(stNnie_config->pcModelName, "r") == HI_NULL){
                printf("[ERRO] %s not exist.", stNnie_config->pcModelName);
                return HI_FAILURE;
            }
            else{
                s32Ret = HI_SUCCESS;
        ...
        if(HI_TRUE == query_value_of_key(cbuf, "thresh=", ctmp)){
            stNnie_config->s32ConfThresh = atof(ctmp);
            printf("[INFO] thresh: %lf\n", stNnie_config->s32ConfThresh);
            continue;
        }
        if(HI_TRUE == query_value_of_key(cbuf, "nms=", ctmp)){
            stNnie_config->s32NmsThresh = atof(ctmp);
            printf("[INFO] nms: %lf\n", stNnie_config->s32NmsThresh);
            continue;
        }
    }
    return s32Ret;
}
HI_S32 hisi_3519av100_nnie_init(){
    HI_S32 s32Ret = HI_SUCCESS;
    /******************************************
     step 1: read config file from disk
     ******************************************/
    SAMPLE_SVP_NNIE_CONFIG_S stNnie_config;
    HI_CHAR* pcConfig = "nnie_config/nnie_config.ini";
    s32Ret = hisi_3519av100_nnie_read_config(pcConfig, &stNnie_config);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,YOLOV3_FAIL,SAMPLE_SVP_ERR_LEVEL_ERROR,
       "Error,hisi_3519av100_nnie_read_config failed!\n");
    HI_CHAR* pcModelName = stNnie_config.pcModelName;
    SAMPLE_SVP_NNIE_CFG_S   stNnieCfg = {0};
    ...
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_ParamInit(&stNnieCfg,&s_stYolov3NnieParam,
...
* function : YOLO Vi->VO
******************************************************************************/
void SAMPLE_SVP_NNIE_YOLOV3_API(void)
{
    hisi_3519av100_nnie_init();
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_low_bandwidth.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_7cls.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_7cls_merged.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_7cls_low_bandwidth.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst_7cls_merged_low_bandwidth.wk";
...
void SAMPLE_SVP_NNIE_YOLOV3(void)
{
    ...
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stYolov3Model);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_YOLOV3_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    ...
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_ParamInit(&stNnieCfg,&s_stYolov3NnieParam,
           &s_stYolov3SoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,END_YOLOV3_1,SAMPLE_SVP_ERR_LEVEL_ERROR,
       "Error,SAMPLE_SVP_NNIE_Rfcn_ParamInit failed!\n");
...
/******************************************************************************
* function : Yolov3 Tiny sample signal handle
******************************************************************************/
void SAMPLE_SVP_NNIE_Tiny_Yolov3_ViVpssVencVo_HandleSig(void)
{
    s_bNnieStopSignal = HI_TRUE;
    if (0 != s_hNnieThread)
    {
    ...
    SAMPLE_SVP_NNIE_Yolov3_Tiny_Deinit(&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam,&s_stTinyYolov3Model);
    memset(&s_stTinyYolov3NnieParam,0,sizeof(SAMPLE_SVP_NNIE_PARAM_S));
    memset(&s_stTinyYolov3SoftwareParam,0,sizeof(SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S));
    memset(&s_stTinyYolov3Model,0,sizeof(SAMPLE_SVP_NNIE_MODEL_S));
...
******************************************************************************/
void SAMPLE_SVP_NNIE_Yolov3_ViVpssVencVo_HandleSig(void)
{
    s_bNnieStopSignal = HI_TRUE;
    if (0 != s_hNnieThread)
    {
...
/******************************************************************************
* function : Yolov3 Tiny software deinit
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_SoftwareDeinit(SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    SAMPLE_SVP_CHECK_EXPR_RET(NULL== pstSoftWareParam,HI_INVALID_VALUE,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error, pstSoftWareParam can't be NULL!\n");
...
/******************************************************************************
* function : hisi3519av100 nnie init
******************************************************************************/
void hisi_3519av100_nnie_init_ssd(void)
{
    HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_ssd_cycle.wk";
    HI_U32 u32PicNum = 1;
    HI_S32 s32Ret = HI_SUCCESS;
...
* function : Yolov3 Tiny software para init
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_SoftwareInit(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ClassNum = 0;
...
void hisi_3519av100_nnie_init_yolov3_tiny(void)
{
    ...
    /*Tiny Yolov3 Load model*/
    SAMPLE_SVP_TRACE_INFO("Tiny Yolov3 Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stTinyYolov3Model);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,TINY_YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    ...
    SAMPLE_SVP_TRACE_INFO("Tiny Yolov3 parameter initialization!\n");
    s_stTinyYolov3NnieParam.pstModel = &s_stTinyYolov3Model.stModel;
    s32Ret = SAMPLE_SVP_NNIE_Yolov3_Tiny_ParamInit(&stNnieCfg,&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,TINY_YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Tiny_Yolov3_ParamInit failed!\n");
    return;
TINY_YOLOV3_FAIL_0:
    SAMPLE_SVP_NNIE_Yolov3_Tiny_Deinit(&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam,&s_stTinyYolov3Model);
    SAMPLE_COMM_SVP_CheckSysExit();
...
* function : hisi3519av100 nnie init
******************************************************************************/
void hisi_3519av100_nnie_init_yolov3(void)
{
    HI_CHAR *pcModelName = "./data/nnie_model/detection/yuv420_inst_yolov3_inst.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/yolov3_inst.wk";
    HI_U32 u32PicNum = 1;
    ...
    /*Yolov2 software parameters are set in SAMPLE_SVP_NNIE_Yolov2_SoftwareInit,
      ...
      SAMPLE_SVP_NNIE_Yolov2_SoftwareInit function are correct*/
    SAMPLE_SVP_TRACE_INFO("Tiny Yolov3 parameter initialization!\n");
    s_stTinyYolov3NnieParam.pstModel = &s_stYolov3Model.stModel;
...
/******************************************************************************
* function : hisi3519av100 nnie detect
******************************************************************************/
void hisi_3519av100_nnie_detect_ssd(void){
    HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/dog_bike_car_300x300.bgr";
    SAMPLE_SVP_NNIE_CFG_S   stNnieCfg = {0};
    stNnieCfg.pszPic = pcSrcFile;
    SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S stInputDataIdx = {0};
    ...
    while(1){
        ...
        SAMPLE_SVP_NNIE_PERF_STAT_END()
        SAMPLE_SVP_NNIE_PERF_STAT_SSD_GR_OP_TIME()
        gettimeofday(&time_end, NULL); //time 0.11s
        /*print result, this sample has 21 classes:
         class 0:background     class 1:plane           class 2:bicycle
         class 3:bird           class 4:boat            class 5:bottle
         class 6:bus            class 7:car             class 8:cat
...
void hisi_3519av100_nnie_detect_yolov3_tiny(void){
    ...
    //HI_CHAR *pcSrcFile = "many_car.yuv";
    HI_CHAR *pcSrcFile = "saved.yuv";
    HI_FLOAT f32PrintResultThresh = 0.0f;
    HI_S32 s32Ret = HI_SUCCESS;
    SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S stInputDataIdx = {0};
    SAMPLE_SVP_NNIE_PROCESS_SEG_INDEX_S stProcSegIdx = {0};
    ...
    f32PrintResultThresh = 0.0f;
    stNnieCfg.pszPic= pcSrcFile;

    /*Fill src data*/
    SAMPLE_SVP_TRACE_INFO("Tiny Yolov3 start!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN_LOOP()
    stInputDataIdx.u32SegIdx = 0;
    stInputDataIdx.u32NodeIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stNnieCfg,&s_stTinyYolov3NnieParam,&stInputDataIdx);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,TINY_YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");

    /*NNIE process(process the 0-th segment)*/
    stProcSegIdx.u32SegIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stTinyYolov3NnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,TINY_YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_YOLOV2_FORWARD_SRC_FLUSH_TIME()
    SAMPLE_SVP_NNIE_PERF_STAT_YOLOV2_FORWARD_PRE_DST_FLUSH_TIME()
...
void hisi_3519av100_nnie_detect_yolov3(void){
    ...
    //HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/street_cars_416x416.bgr";
    //HI_CHAR *pcSrcFile = "./many_car_416_416.bgr";
    HI_CHAR *pcSrcFile = "./dog_bike_car.jpg.yuv";
...
* function : show SSD sample(image 300x300 U8_C3)
******************************************************************************/
void SAMPLE_SVP_NNIE_SSD_LOOP_SPLIT(void)
{
    hisi_3519av100_nnie_init_ssd();
    hisi_3519av100_nnie_detect_ssd();
}

/******************************************************************************
* function : show Yolov3 Tiny sample(image 416x416 U8_C3)
******************************************************************************/
void SAMPLE_SVP_NNIE_YOLOV3_TINY_READ_FILE_LOOP(void)
{
    hisi_3519av100_nnie_init_yolov3_tiny();
    while(1){
        struct timeval time_start, time_end;
        gettimeofday(&time_start, NULL);
...
******************************************************************************/
void SAMPLE_SVP_NNIE_YOLOV3_READ_FILE_LOOP(void)
{
    hisi_3519av100_nnie_init_yolov3();
    while(1){
        struct timeval time_start, time_end;
        gettimeofday(&time_start, NULL);
...
* function : show SSD sample(image 300x300 U8_C3)
******************************************************************************/
void SAMPLE_SVP_NNIE_SSD_LOOP(void)
{
    HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/dog_bike_car_300x300.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_ssd_cycle.wk";
    HI_U32 u32PicNum = 1;
...
/******************************************************************************
* function : show Tiny Yolov3 sample
******************************************************************************/
void SAMPLE_SVP_NNIE_Tiny_Yolov3(void)
{
    HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/street_cars_416x416.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_yolov3_tiny_inst.wk";
    HI_U32 u32PicNum = 1;
...
{
    HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/street_cars_416x416.bgr";
    //HI_CHAR *pcSrcFile = "./street_cars.bgr";
...
* function : Yolov3 Tiny sample signal handle
******************************************************************************/
void SAMPLE_SVP_NNIE_Tiny_Yolov3_HandleSig(void)
{
    SAMPLE_SVP_NNIE_Yolov3_Tiny_Deinit(&s_stTinyYolov3NnieParam,&s_stTinyYolov3SoftwareParam,&s_stTinyYolov3Model);
    memset(&s_stTinyYolov3NnieParam,0,sizeof(SAMPLE_SVP_NNIE_PARAM_S));
    memset(&s_stTinyYolov3SoftwareParam,0,sizeof(SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S));
